...
mp.options.read_options(options, "evafast", function()
end)
...
local function speed_transition(current_speed, target_speed)
    ...
    while adjusted_speed ~= target_speed do
        ...
        if (current_speed < target_speed and adjusted_speed > target_speed) or
            (current_speed > target_speed and adjusted_speed < target_speed) then
...
local function ensure_timer(reset)
    if not reset and speed_timer:is_enabled() then
        return
    end
...
local function adjust_speed()
    ...
    if math.floor(target_speed * 1000 + 0.5) == math.floor(current_speed * 1000 + 0.5) then
        if forced_slowdown or (not toggled and
            (not speedup or options.subs_speed_cap == options.speed_cap or (not has_subtitle and not speedup_target))) then
            speed_timer:kill()
            if speedup_target ~= nil then
                evafast_slowdown()
            end
...
mp.add_key_binding("LEFT", "evafast-rewind", evafast_rewind, {
    repeatable = true,
    complex = true
})
