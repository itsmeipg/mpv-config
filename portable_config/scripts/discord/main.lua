...
local function get_platform() -- Taken from uosc
	local platform = mp.get_property_native('platform')
	if platform then
		if platform == "windows" or platform == "darwin" then return platform end
	else
...
local binary_path = mp.command_native({"expand-path", "~~/scripts/discord/bin"}) .. "/" .. (platform == "windows" and "windows.exe" or platform)
